...
public class PerformanceEndpoint {
    ...
    public List<PerformanceDto> findAllByDate(PaginationDto paginationDto) {
        LOGGER.info("GET /api/v1/events");
        return performanceMapper.performanceListToPerformanceDtoList(performanceService.findAllByDate(paginationMapper.paginationDtoToPageable(paginationDto)));
    }

    @PermitAll
    @GetMapping(value = "/{id}")
    @Operation(summary = "Get a specific event")
    public PerformanceDto find(@PathVariable Long id) {
